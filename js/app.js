...
const ICON_SUFFIX = "=s180-rw"; // Google Play 图标尺寸参数
...
const iconUrl = (app) => app.icon + ICON_SUFFIX;
...
function renderApps(filter = "all") {
  ...
  appGrid.innerHTML = list
    .map(
      (app, i) => `
    <article class="app-card" data-id="${app.id}" style="animation-delay:${i * 0.05}s" tabindex="0" role="button" aria-label="查看 ${app.shortName} 详情">
      <div class="card-top">
        <img class="app-icon" src="${iconUrl(app)}" alt="${app.shortName} 图标" loading="lazy"
             onerror="this.onerror=null;this.src='${app.icon}'">
        <div class="card-title-wrap">
          <h3 class="app-name">${app.shortName}</h3>
          <p class="app-category">${app.categoryLabel} ${ratingText(app)}</p>
        </div>
      </div>
      <p class="app-desc">${app.shortDesc}</p>
      <div class="card-foot">
        <span class="app-downloads">⬇ ${displayDownloads(app)} 次下载</span>
...
function openModal(app) {
  ...
  $("modalDesc").textContent = app.shortDesc;
  $("modalPlayBtn").href = playUrl(app);

  const chips = [
    `<span class="m-chip">📂 ${app.categoryLabel}</span>`,
    `<span class="m-chip">⬇ ${displayDownloads(app)} 次下载</span>`,
    `<span class="m-chip">👤 内容分级 ${app.contentRating}</span>`,
  ];
  if (app.rating > 0) chips.unshift(`<span class="m-chip hl">★ ${app.rating.toFixed(1)} 好评</span>`);
  ...
  carouselTrack.innerHTML = featuredApps
    ...
    <div class="slide">
      <div class="slide-inner">
        <img class="slide-icon" src="${iconUrl(app)}" alt="${app.shortName} 图标" loading="lazy"
             onerror="this.onerror=null;this.src='${app.icon}'">
        <div class="slide-body">
          <span class="slide-badge">🔥 主打推荐</span>
          <h3 class="slide-name">${app.shortName}</h3>
          <p class="slide-desc">${app.shortDesc}</p>
          <div class="slide-meta">
            <span>⬇ ${displayDownloads(app)} 次下载</span>
            ${
              app.rating > 0
                ? `<span class="slide-rating">★ ${app.rating.toFixed(1)} 好评</span>`
                : `<span>📂 ${app.categoryLabel}</span>`
            }
          </div>
          <div class="slide-actions">
            <button class="btn btn-slide-detail" data-featured-id="${app.id}">查看详情</button>
            <a class="btn btn-slide-play" href="${playUrl(app)}" target="_blank" rel="noopener">获取 Google Play</a>
          </div>
        </div>
      </div>
    </div>`
